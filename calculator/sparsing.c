...
_int8 check(char c){
    if ((c >= 0x30 && c <= 0x39) || c == 0x2A || c == 0x2B || c == 0x21 || c == 0x2F || c == 0x25 || c == 0x2D){
        return 0x0;
    }
    return 0x1;
}
...
_int8 parse_for_ll(char *s){
    _uint8 i = 0x0, j = 0x0, t = 0x0, x = 0x0; // i iterator over the buffer j counter for numbers t counter for symbols x counter for digits of a number
    _int64 *numbers = (_uint64*)malloc(sizeof(_uint64) * MAX_OPERANDS);
    char *p_digits = (char*)malloc(sizeof(char)*LEN_LL); // MAX_INT64 is 19 digits
    ...
    while (*(s+i) != 0x0){
        ...
        while (*(s+i+x) >= 0x30 && *(s+i+x) <= 0x39){ // 0(n)->second loop will only be accessed in case of a digit is found
            if (x >= LEN_LL) {goto _overflow;} // too many digits
            *(p_digits+x) = (s+i+x);
            x++;
            if (*(s+i+x) > 0x39 || *(s+i+x) < 0x30){ // no more digits
                
                *(p_digits+x) = '\0'; // set null terminator
                *(numbers+j) =  (_int64)strtoll(p_digits, NULL, 0x0A); // lets conv the char pntr to int
                x = 0x0;
                j++;
            }    
        }
    ...
    free(symbols); free(p_digits); free(numbers);
    return 0;
    _overflow:
        free(symbols); free(p_digits); free(numbers);
        return 1;
...
_int8 parse_for_f(char *s){
    ...
    while (*(s+i) != 0x0){
        ...
        //found symbol
        if (*(s+i) == 0x2A || *(s+i) == 0x2B || *(s+i) == 0x2E || *(s+i) == 0x21 || *(s+i) == 0x2F || *(s+i) == 0x25 || *(s+i) == 0x2D){
            *(symbols+t) = *(s+i);
            t++;
        }
    i++;
